       ...
       DOMCacheGetOrSet('Auto4').style.borderColor = data.autoActive[4] ? '#438043' : '#963a2e'
        ...
        if(data.currentSubTab[1] === 0) {
            ...
            DOMCacheGetOrSet('splitImage').style.backgroundColor = gainMult.gt(D(1)) ? '#379337' : '#934237'
                ...
                DOMCacheGetOrSet('lepUnlock2').innerHTML = data.leptonUnlocks[1] === true ? `Unlocked<br>Power Capacity Buff: ${format(leptonBoosts[0])}x` : `Unlock Petroleum Generator<br>Cost: 250 Muons`
                DOMCacheGetOrSet('lepUnlock3').innerHTML = data.leptonUnlocks[2] === true ? `Unlocked<br>Fuel Production Buff: ${format(leptonsBoosts[1])}x` : `Unlock Natural Gas Generator<br>Cost: 200 Taus`
        }
