...
function updateAccelStuff() {
   ...
   for(let i = 0; i < quarkBoosts.length; i++) {
       ...
       quarkBoosts[i] = quarkBoosts[i].divide(antimatterEffects[2])
       quarkBoosts[i] = D(1).plus(quarkBoosts[i])
   }
}
...
function buyLepton(a) {
    switch(a) {
        ...
        case 1:
            ...
            break
        case 2:
            if(data.particles[1].taus.lt(D(100)) || data.leptonUnlocks[a] === true) return 
            data.particles[1].taus = data.particles[1].taus.sub(D(100))
            data.leptonUnlocks[a] = true
            break
    }
}
